...
export const registrarCobro = async (datos) => {
  ...
  // 1. Calcular total
  const total = facturas.reduce((sum, f) => sum + Number(f.importe || 0), 0);
  ...
  const nuevoCobro = new Cobro({
    ...
    // ✅ SOLUCIÓN: incluir facturaId
    facturasPagadas: facturas.map((f) => ({
      facturaId: f._id, // ✅ requerido por tu schema
      numeroComprobante: f.numeroDeComprobante,
      tipo: f.tipo || "factura",
      detalle: f.detalle,
...
export const obtenerCobroPorId = async (id) => {
  return Cobro.findById(id).populate("cliente", "nombre apellido dni").lean();
};
